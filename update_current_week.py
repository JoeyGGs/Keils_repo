"""
Apply schedule from image to current week (12/15 - 12/21)
"""
...
# Current week dates (12/15 - 12/21)
mon = date(2025, 12, 15)
tue = date(2025, 12, 16)
wed = date(2025, 12, 17)
thu = date(2025, 12, 18)
...
# Remove any existing shifts for current week
shifts = [s for s in shifts if not (mon <= s.date <= sun)]
...
# Michael - 32 hours (OFF Mon-Wed, work Thu-Sun)
...
add_shift("EMP003", "Lilah", tue, is_off=True)
...
add_shift("EMP003", "Lilah", sat, is_off=True)
...
add_shift("EMP004", "Erin", wed, is_off=True)
add_shift("EMP004", "Erin", thu, 5, 0, 13, 30, "OP")  # 5-1:30 OP
add_shift("EMP004", "Erin", fri, 5, 0, 13, 30, "CK")  # 5-1:30 CK
add_shift("EMP004", "Erin", sat, is_off=True)
add_shift("EMP004", "Erin", sun, is_off=True)

# Rashid - 16 hours
add_shift("EMP005", "Rashid", mon, is_off=True)
...
add_shift("EMP005", "Rashid", fri, is_off=True)
add_shift("EMP005", "Rashid", sat, 5, 0, 13, 30, "BAR")  # 5-1:30 BAR
add_shift("EMP005", "Rashid", sun, is_off=True)
...
add_shift("EMP006", "Haley", fri, is_off=True)
...
add_shift("EMP007", "Ellen", tue, is_off=True)
...
add_shift("EMP007", "Ellen", sun, is_off=True)

# Mirna - 40 hours
add_shift("EMP008", "Mirna", mon, 6, 0, 14, 30, "FR")  # 6-2:30 FR
add_shift("EMP008", "Mirna", tue, 6, 0, 14, 30, "FR")  # 6-2:30 FR
add_shift("EMP008", "Mirna", wed, is_off=True)
add_shift("EMP008", "Mirna", thu, is_off=True)
...
# Gianni - 16 hours
add_shift("EMP009", "Gianni", mon, is_off=True)
add_shift("EMP009", "Gianni", tue, is_ro=True)
add_shift("EMP009", "Gianni", wed, 12, 0, 20, 30, "PM")  # 12-8:30 PM
add_shift("EMP009", "Gianni", thu, is_off=True)
add_shift("EMP009", "Gianni", fri, 12, 0, 20, 30, "PM")  # 12-8:30 PM
add_shift("EMP009", "Gianni", sat, is_off=True)
add_shift("EMP009", "Gianni", sun, is_off=True)
...
# Richard - 24 hours
add_shift("EMP012", "Richard", mon, is_off=True)
add_shift("EMP012", "Richard", tue, is_off=True)
...
add_shift("EMP013", "Simon", wed, is_off=True)
add_shift("EMP013", "Simon", thu, is_off=True)
add_shift("EMP013", "Simon", fri, 13, 0, 21, 30, "PM")  # 1-9:30 PM
add_shift("EMP013", "Simon", sat, is_off=True)
add_shift("EMP013", "Simon", sun, is_off=True)

# Save shifts
data_store.save_shifts(shifts)

# Count shifts for current week
current_week_shifts = [s for s in shifts if mon <= s.date <= sun]
print(f"✓ Updated current week (12/15 - 12/21) with {len(current_week_shifts)} shifts")
print("Schedule now matches the image!")
